...
- Expects completed changes to be committed and pushed to the remote once the work is done ("commitant push this"; "commit and push this"; "commit and push"). Confirmed again as a standalone command: stage everything with `git add -A` (including the user's own manual edits and new images), commit, and `git push` to `main` without asking for confirmation. Confidence: 0.75
